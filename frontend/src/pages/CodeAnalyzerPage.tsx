...
import { DsaVisualizerPage } from '@/features/dsa-visualizer/DsaVisualizerPage'

type Mode = 'ai-analysis' | 'dsa-visualizer'
...
export function CodeAnalyzerPage() {
  const [mode, setMode] = useState<Mode>('ai-analysis')
  ...
  if (mode === 'dsa-visualizer') {
    return (
      <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
        <div style={{ display: 'flex', borderBottom: '1px solid var(--color-border-subtle)', background: 'var(--color-surface-container-low)' }}>
          <button style={tabStyle(false)} onClick={() => setMode('ai-analysis')}>
            <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 0" }}>code_blocks</span>
            AI Analysis
          </button>
          <button style={tabStyle(true)} onClick={() => setMode('dsa-visualizer')}>
            <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>slow_motion_video</span>
            DSA Visualizer
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      <div style={{ display: 'flex', borderBottom: '1px solid var(--color-border-subtle)', background: 'var(--color-surface-container-low)' }}>
        <button style={tabStyle(true)} onClick={() => setMode('ai-analysis')}>
          <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 0" }}>code_blocks</span>
          AI Analysis
        </button>
        <button style={tabStyle(false)} onClick={() => setMode('dsa-visualizer')}>
          <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>slow_motion_video</span>
          DSA Visualizer
        </button>
      </div>
