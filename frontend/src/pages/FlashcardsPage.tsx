...
export function FlashcardsPage() {
  ...
  // Limit guests to maximum 20 cards trial session
  const cards = !isAuthenticated ? rawCards.slice(0, 20) : rawCards
  const pagination = response?.pagination
  const totalCards = !isAuthenticated ? Math.min(20, pagination?.total ?? 0) : (pagination?.total ?? 0)
  ...
  return (
    <>
      <SEOHead
        title="Flashcards"
        description="Spaced-repetition technical interview flashcards for software engineering, system design, and coding concepts."
      />
      <div className="px-6 py-8 max-w-3xl mx-auto">
        ...
        {!isAuthenticated && (
          <div className="p-4 rounded-xl mb-6 flex items-center justify-between gap-4 animate-fade-up" style={{ background: 'var(--color-primary-container)', border: '1px solid var(--color-primary)' }}>
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[20px]" style={{ color: 'var(--color-primary)' }}>lock</span>
              <p className="text-xs font-semibold" style={{ color: 'var(--color-on-primary-container)' }}>
                Guest Trial Mode: Limited to 20 flashcards. Sign in to unlock full decks and save progress.
              </p>
            </div>
            <Link to="/login" className="px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap" style={{ background: 'var(--color-primary)', color: 'var(--color-on-primary-fixed)' }}>
              Sign In
            </Link>
          </div>
        )}
