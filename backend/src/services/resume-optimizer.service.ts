...
function cleanJson(str: string): string {
  ...
  if (content.startsWith('```json')) content = content.replace(/^```json\s*/, '').replace(/\s*```$/, '')
  else if (content.startsWith('```')) content = content.replace(/^```\w*\s*/, '').replace(/\s*```$/, '')
  return content
}
...
export class ResumeOptimizerService {
  ...
  async analyzeJobDescriptionStreamed(
    resumeId: string,
    ...
    let jobData: any
    try {
      jobData = JSON.parse(cleanJson(jobJson))
    } catch {
      jobData = { rawText: jobDescription, parseError: 'AI returned invalid JSON' }
    }
    ...
    const gapJson = await streamAI([
      { role: 'system', content: GAP_ANALYSIS_SYSTEM_PROMPT },
      { role: 'user', content: getGapAnalysisPrompt(resumeJson, JSON.stringify(jobData)) },
    ], emit)

    let gapData: any
    try {
      gapData = JSON.parse(cleanJson(gapJson))
    } catch {
      gapData = { resumeMatchPercentage: 0, atsScore: 0, missingSkills: [], missingKeywords: [], improvementSuggestions: [] }
    }
  ...
  async optimizeResumeStreamed(
    ...
    const optimizationJson = await streamAI([
      { role: 'system', content: RESUME_OPTIMIZATION_SYSTEM_PROMPT },
      { role: 'user', content: getResumeOptimizationPrompt(resumeJson, JSON.stringify(suggestions), JSON.stringify(suggestions)) },
    ], emit, 6144)

    let optimizedData: any
    try {
      optimizedData = JSON.parse(cleanJson(optimizationJson))
    } catch {
  ...
  async generateCoverLetterStreamed(
    resumeId: string,
    emit: (content: string) => void,
    companyName?: string,
    jobTitle?: string
    ...
    const coverLetterJson = await streamAI([
    ...
    try {
      coverLetterData = JSON.parse(cleanJson(coverLetterJson))
    } catch {
